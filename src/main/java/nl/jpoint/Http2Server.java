...
/**
 * Original source:
 * https://github.com/fstab/http2-examples/blob/master/undertow-http2-servlet-example/src/main/java/de/consol
...
public class Http2Server {
	...
	public static void main(final String[] args) throws Exception {
		...
		DeploymentInfo servletBuilder = deployment().setClassLoader(pushServlet.class.getClassLoader())
				.setContextPath(MYAPP).setDeploymentName("hello-world.war")
				.setResourceManager(new FileResourceManager(new File("src/main/webapp"), 1024)).addServlets(
						servlet("pushServlet", pushServlet.class).addInitParam("message", "push")
								.addMapping("/api/push"),
						servlet("pushedMessage", pushedMessage.class).addInitParam("message", "message")
		...
		SSLContext sslContext = createSSLContext(loadKeyStore("/kserver.keystore"), loadKeyStore("/tserver.keystore"));
		Undertow server = Undertow.builder().setServerOption(UndertowOptions.ENABLE_HTTP2, true)
				.setServerOption(UndertowOptions.HTTP2_SETTINGS_ENABLE_PUSH, true)
				.addHttpsListener(8443, bindAddress, sslContext).setHandler(path).build();
